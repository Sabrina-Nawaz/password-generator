...
function generatePassword() {
  ...
  if (!lCase && !uCase && !numbers && !specialChar) {
  ...
  if (lCase) passChars = passChars.concat(lowerCaseArr);
  if (uCase) passChars = passChars.concat(upperCaseArr);
  if (numbers) passChars = passChars.concat(numbersArr);
  if (specialChar) passChars = passChars.concat(specialsArr);
...
// Add event listener to generate button. When the Generate Button is clicked, then the application prepares to write out a password based on the criterias selected. 
